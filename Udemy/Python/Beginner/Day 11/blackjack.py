...
def game():
    ...
    if player_sum == 21:
        print(
            f"Your starting cards are {player_cards}, which adds up to 21, you win!")
        playAgainPrompt()
    ...
    def s_behavior():
        print(
            f"Your final value of your hand is {player_sum}. Computer is now drawing...")
        computer_sum = sum(computer_cards)
        while(computer_sum < 16):
            ...
            time.sleep(1)
            print(
                f"Computer drew {last_computer_card}. Computer total: {computer_sum}\t Computer cards: {computer_cards} ")
            compare_scores()
    ...
    def h_behavior():
        ...
        if player_sum > 21:
            print(
                f"You went over 21 (drew a {last_card}). You busted with a {player_sum}!")
            playAgainPrompt()
        if player_sum == 21:
            print("You drew to 21! you win!")
            playAgainPrompt()
        print(
            f"You drew {last_card}: \tNew value is {player_sum} (Cards on hand: {player_cards}")
